...
without modifying the image, e.g. when a legacy app writes to a
file instead of stdout.

A special sidecar is picking up the logs from the library
...
structured data (presumably MessagePack) and is uncapable of
...
So a simple log forwarder was designed and programmed in Go.
Reading from the socket, it picks up the logs and packages
them in a JSON record, enriched by some metadata. It then
writes that to a) stdout, b) a fluentd tcp sink or c) both.
...
to pick it up and forward it. Simple, effective, gets the job done.
